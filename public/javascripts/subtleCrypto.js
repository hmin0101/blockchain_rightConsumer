...
async function exportKey() {
    if (encryptKey === null) {
        ...
        const exported_pri = await wrapCryptoKey("private", encryptKey.privateKey, enc_password);
        ...
        // Wrapping public key
        const exported_pub = await wrapCryptoKey("public", encryptKey.publicKey, enc_password);
...
/* Wrapping key For Export */
async function wrapCryptoKey(type, key, enc_pw) {
...
async function createSignature(name) {
    if (signKey.publicKey === null) {
        ...
        return window.crypto.subtle.sign({ name: "RSA-PSS", saltLength: 32 }, signKey.privateKey, enc_data);
    }
}
